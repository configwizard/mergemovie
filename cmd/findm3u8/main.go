...
func main() {
	url := "" //provide a url to a page you think has a m3u8 video type
	urls, err := findM3U8Urls(url)
	if err != nil {
	...
	for _, m := range urls {
		fmt.Println("Found master URL:", m)
		variantUrls, err := retrieveVariants(m)
		if err != nil {
			fmt.Println("Error:", err)
			return
		}

		for _, v := range variantUrls {
			fmt.Println("Found variant URL:", v)
			variantURL, err := combineURL(m, v)
			if err != nil {
				fmt.Println("Error combining URLs:", err)
				continue
			}
			if err := downloadSegments(variantURL); err != nil {
				fmt.Println("Error:", err)
				return
			}
			break
